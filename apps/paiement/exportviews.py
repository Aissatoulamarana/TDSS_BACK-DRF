...
def export_pdf(name, data, design_width):
    ...
    d.add(
        String(
            320,
            60,
            f"Date : {print_date.format('d/m/Y H:m')}",
            fontSize=11,
            fontName="Helvetica",
...
def exportpdf_agencies_view(request):
    ...
    for agency in agencies:
        data.append(
            [f"{counter}", f"{agency.code}", f"{agency.region}", f"{agency.name}"]
        )
        counter += 1
...
def exportpdf_payments_view(request):
    ...
    for payment in payments:
        ...
        data.append(
            [
                f"{counter}",
                f"N° 00{payment.id}/{pay_date.format('Y')}",
                f"{payment.created_by.profile.name}",
                f"{payment.facture_ref.client.name}",
                f"{payment.payer}",
                f"{to_amount(payment.amount)} {payment.devise.sign}",
                f"{pay_date.format('d/m/Y H:m')}",
            ]
        )
        counter += 1
...
def exportpdf_bills_view(request):
    name = "Factures"
    bills = Facture.objects.select_related('client', 'declaration_ref').all().order_by("id")
    data = [
        [
            "N°",
            "Référence",
    ...
    for bill in bills:
        ...
        declaration_date = DateFormat(bill.declaration_ref.created_on)
        data.append(
            [
                f"{counter}",
                f"00{bill.id}/{pay_date.format('Y')}",
                f"00{bill.declaration_ref.id}/{declaration_date.format('Y')}",
                f"{bill.declaration_ref.title}",
                f"{bill.client.name}",
                f"{to_amount(bill.amount)} {bill.devise.sign}",
                f"{bill.get_status_display()}",
                f"{pay_date.format('d/m/Y H:m')}",
            ]
        )
        counter += 1
...
def exportpdf_declarations_view(request):
    ...
    for declaration in declarations:
        ...
        data.append(
            [
                f"{counter}",
                f"N° 00{declaration.id}/{pay_date.format('Y')}",
                f"{declaration.created_by.profile.name}",
                f"{declaration.title}",
                f"{declaration.employees.count()}",
                f"{declaration.get_status_display()}",
                f"{pay_date.format('d/m/Y H:m')}",
            ]
        )
        counter += 1
...
def exportxlsx_agencies_view(request):
    ...
    return FileResponse(
        file, as_attachment=True, filename=f"export_excel_{name.lower()}.xlsx"
    )
...
def exportxlsx_bills_view(request):
    ...
    name = "Factures"
    bills = (
        Facture.objects.select_related("client", "declaration_ref").all().order_by("id")
    )
    data = []
    counter = 1
    for bill in bills:

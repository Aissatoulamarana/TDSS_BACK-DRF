...
class Permit(models.Model):
    ...
    type = models.CharField(
        max_length=30, choices=[("A", "A"), ("B", "B"), ("C", "C")], default="A"
    )
...
class Declaration(models.Model):
    ...
    status = models.CharField(
        max_length=30, choices=status_choices, default="unsubmitted"
    )
    ...
    employees = models.ManyToManyField(
        ...
        through="DeclarationEmployee",
        related_name="alldeclarations",
        related_query_name="alldeclaration",
    )
    # image = models.ImageField(_("image"), upload_to=Base.change_image_name, max_length=500)
...
class Employee(models.Model):
    status_choices = [("unenrolled", "Non enrôlé"), ("enrolled", "Enrôlé")]
    declaration = models.ForeignKey(
        Declaration, on_delete=models.PROTECT, related_name="employee_declarations"
    )  # sera supprimer pour garder seuelment le many-to-many
    ...
    job_category = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="employee_job_categories",
    )
...
class DeclarationEmployee(models.Model):
    ...
    declaration = models.ForeignKey(
        ...
        on_delete=models.PROTECT,
        related_name="declarationemployees",
        related_query_name="declarationemployee",
    )
...
class Facture(models.Model):
    status_choices = [("unpaid", "Impayée"), ("paid", "Payée")]
    reference = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    declaration_ref = models.ForeignKey(
        Declaration, on_delete=models.PROTECT, related_name="declaration_factures"
    )
    ...
    devise = models.ForeignKey(
        Devise, on_delete=models.PROTECT, related_name="facture_devises"
    )
    comment = models.TextField(max_length=255, blank=True, null=True)
    created_by = models.ForeignKey(CustomUser, on_delete=models.PROTECT)
    created_on = models.DateTimeField(auto_now_add=True)
...
class Payer(models.Model):
    ...
    country_origin = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="payer_countries",
    )
    employer = models.ForeignKey(
        Profile, on_delete=models.PROTECT, blank=True, null=True
    )
    job = models.ForeignKey(
        Job, on_delete=models.PROTECT, blank=True, null=True, related_name="payer_jobs"
    )
...
class Payment(models.Model):
    reference = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    facture_ref = models.ForeignKey(
        Facture, on_delete=models.PROTECT, blank=True, null=True
    )
    type = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="payment_permits_types",
    )
    payer = models.ForeignKey(
        Payer, on_delete=models.PROTECT, related_name="payment_payers"
    )
    amount = models.DecimalField(max_digits=15, decimal_places=2)
    devise = models.ForeignKey(
        Devise, on_delete=models.PROTECT, related_name="payment_devises"
    )
    comment = models.TextField(max_length=255, blank=True, null=True)
    created_by = models.ForeignKey(CustomUser, on_delete=models.PROTECT)
    created_on = models.DateTimeField(auto_now_add=True)

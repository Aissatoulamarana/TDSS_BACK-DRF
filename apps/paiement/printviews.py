...
def bill_receipt_view(request, bill_id):
    ...
    pdf.drawImage('apps/static/assets/img/bill/conditions_aguipee_tdss.png', 55, 140, 280, 200, showBoundary=False)
    # pdf.line(50, 750, 550, 750)
    pdf.setFontSize(20, leading=None)
    pdf.drawString(220, 700, f"Facture N° 00{facture.id}/{date.format('Y')}")
    pdf.setFontSize(10, leading=None)
    pdf.setFillColor(colors.darkblue)
    pdf.drawString(420, 650, f"CLIENT")
    pdf.line(420, 647, 455, 647)
    pdf.setFillColor(colors.black)
    pdf.drawString(420, 630, f"{client.name}")
    pdf.drawString(420, 615, f"Tél : {to_amount(client.contact)}")
    pdf.drawString(420, 600, f"{client.adresse}")

    pdf.setFillColor(colors.darkred)
    pdf.drawString(55, 645, f"Date facture : ")
    pdf.drawString(55, 632, f"Déclaration N° : ")
    pdf.drawString(55, 619, f"Date déclaration : ")
    pdf.setFillColor(colors.black)
    pdf.drawString(120, 645, f"{date.format('d/m/Y')}")
    pdf.drawString(130, 632, f"00{facture.declaration_ref.id}/{DateFormat(facture.declaration_ref.created_on).format('Y')}")
    pdf.drawString(140, 619, f"{DateFormat(facture.declaration_ref.created_on).format('d/m/Y')}")
